document.addEventListener('DOMContentLoaded', function () {
  ...
  $('button').click(function () {
    $.ajax({
      ...
      url: url_prefix + ":5001/api/v1/places_search/",
      data: JSON.stringify({"amenities": places_filter}),
  ...
  $('input').each(function (idx, ele) {
    ...
    $(ele).change(function () {
      ...
      if (this.checked) {
        ...
        places_filter.push(id)
      } else {
        $('span#' + id).remove();
        places_filter.splice(places_filter.indexOf(id), 1);
      }
  ...
  $(function () {
    $.ajax({
      type: 'GET',
      url: url_prefix + ":5001/api/v1/status/",
      success: function (data) {
...
function populatePlaces(data){
	$.ajax({
          type: 'GET',
          url: url_prefix + ':5001/api/v1/users/',
